...
//var_dump($_instagram_json);
        if( empty($_instagram_json) || !isset($response['response']['code']) ){
            ...
            while( !empty($repeat) && count($photos)<$num ){
                ...
                foreach( $data as $key=>$imageinfo ){
                    ...
                    if( empty($record[ $url ]) && count($photos)<$num ){
                        ...
                        elseif( 'VIDEO' == $imageinfo['media_type'] ){ // Filter out videos
                            // Do nothing
                        }
                        elseif( 'CAROUSEL_ALBUM' == $imageinfo['media_type'] ){ // Filter out videos
                            // Do nothing
                        }elseif( !empty($blocked) && !empty($imageinfo['user']) && ((!empty($imageinfo['username'])&&in_array($imageinfo['username'],$blocked))||(!empty($imageinfo['user']['id'])&&in_array($imageinfo['user']['id'],$blocked))) ){
                            ...
                            $the_photo['image_link'] = (string) isset($imageinfo['permalink'])?$imageinfo['permalink']:'';

                            $the_photo['image_title'] = (string) isset($imageinfo['caption'])?$imageinfo['caption']:'';
                            ...
                            $the_photo['image_user_fullname'] = (string) isset($imageinfo['user']['full_name'])?$imageinfo['user']['full_name']:'';
                            $the_photo['image_username'] = (string) isset($imageinfo['username'])?$imageinfo['username']:'';
                            $the_photo['image_profile_picture'] = (string) isset($imageinfo['user']['profile_picture'])?$imageinfo['user']['profile_picture']:'';
                            $the_photo['image_likes'] = (string) isset($imageinfo['likes']['count'])?$imageinfo['likes']['count']:'';
                            $the_photo['image_comments'] = (string) isset($imageinfo['comments']['count'])?$imageinfo['comments']['count']:'';
                            $the_photo['image_created'] = (string) isset($imageinfo['timestamp'])?$imageinfo['timestamp']:'';
                            $the_photo['image_type'] = (string) isset($imageinfo['media_type'])?$imageinfo['media_type']:'';


                            $the_photo['image_caption'] = "";

                            $the_photo['image_source'] = (string) $url;
                            if($the_photo['image_type'] == 'IMAGE') {

                                $the_photo['image_original'] = (string) isset($imageinfo['media_url'])?$imageinfo['media_url']:$the_photo['image_source'];

                            }
                            elseif($the_photo['image_type'] == 'VIDEO') {
                                $the_photo['image_original'] = (string) isset($imageinfo['thumbnail_url'])?$imageinfo['thumbnail_url']:$the_photo['image_source'];
                ...
                $next_url = (isset($_instagram_json['paging']['next'])) ? $_instagram_json['paging']['next'] : null;
                // Previous code  if( count($photos)<$num && !empty($next_url) ){
                if(!empty($next_url) ){
     ...
     *  @ Updated 1.2.7.1
     */
    function get_instagram_request( $token, $client_id, $user_id, $num = 25 ){
        $request = false;
        $options = $this->get_private('options');
        ...
        if( isset($options['instagram_source']) ){
            switch ($options['instagram_source']) {
                case 'user_recent':
                    $request = 'https://graph.instagram.com/'.$user_id.'/media?fields=id,media_type,media_url,username,timestamp,caption,thumbnail_url,permalink&access_token='.$token.'&limit='.$num;
...
class PhotoTileForInstagramBot extends PhotoTileForInstagramBotTertiary{
    ...
    function display_cascade(){
        ...
        $this->add('<div class="AlpinePhotoTiles_breakline"></div>');
        // $this->add("<button class='Alpine-load-more'>Load more</button>");
        $this->add('<div class="AlpinePhotoTiles_breakline"></div>');
        $this->add_user_link($wid);
    ...
    function add_image($i,$siteurl,$wid,$src,$shadow,$border,$curves,$highlight,$onContextMenu,$ssl=false,$pin=false,$css=""){
        ...
        $imagesrc = $this->get_photo_info($i,'image_original');
        ...
        if( $pin ){
            $original = $this->get_photo_info($i,'image_source');
            if( $ssl ){
                $original = str_replace("http:", "https:", $original, $temp = 1);
            }
    ...
    function get_link($i,$imagetitle,$src,$ssl=false){
        ...
        $image_user_fullname = $this->get_photo_info($i,'image_user_fullname');
        $image_username = $this->get_photo_info($i,'image_username');
        $image_profile_picture = $this->get_photo_info($i,'image_profile_picture');
        $image_likes = $this->get_photo_info($i,'image_likes');
        $image_comments = $this->get_photo_info($i,'image_comments');
        $image_created = $this->get_photo_info($i,'image_created');

        $image_link = $this->get_photo_info($i,'image_link');
        $image_type = $this->get_photo_info($i,'image_type');

        $image_created = explode("T", $image_created);

        $image_created = date(get_option( 'date_format' ), strtotime($image_created[0]));
        $add_data_att = "data-user-fullname='".$image_user_fullname."' data-username='".$image_username."' data-profile_picture='".$image_profile_picture."' data-image_likes='".$image_likes."' data-image_comments='".$image_comments."' data-image_link='".$image_link."' data data-image_created='".$image_created."' data-image_type='".$image_type."'";


        if( 'fancybox' == $ilink ){
            $originalurl = $this->get_photo_info($i,'image_source');
            if( $ssl ){ $originalurl = str_replace("http:", "https:", $originalurl, $temp = 1); }
            if( !empty($originalurl) ){
                $this->add('<a href="' . $originalurl . '" class="AlpinePhotoTiles-link AlpinePhotoTiles-lightbox" title=" '. $imagetitle .' " alt=" '. $imagetitle .' " ' .$add_data_att. '>');
